# Code generated by `ubx sdk gen` from datadog_schedule_on_call_responders_included. DO NOT EDIT.
from __future__ import annotations

import dataclasses
from typing import Any

import ubx_sdk as ubx

@dataclasses.dataclass
class OnCallRespondersIncluded_Data_Attributes:
    # The timestamp the responders were resolved at.
    scheduled_at: Any = None

@dataclasses.dataclass
class OnCallRespondersIncluded_Data_Relationships_Responders_Data:
    id: Any = None
    type: Any = None

@dataclasses.dataclass
class OnCallRespondersIncluded_Data_Relationships_Responders:
    # Array of references to the responder groups included in the response.
    data: Any = None

@dataclasses.dataclass
class OnCallRespondersIncluded_Data_Relationships_Schedule:
    # Represents a reference to the schedule this on-call responders lookup was performed for.
    data: Any = None

@dataclasses.dataclass
class OnCallRespondersIncluded_Data_Relationships:
    # Defines the list of per-position (previous, current, next) responder groups for the schedule.
    responders: Any = None
    # Defines the relationship to the schedule this on-call responders lookup was performed for.
    schedule: Any = None

@dataclasses.dataclass
class OnCallRespondersIncluded_Data:
    # Attributes for a schedule's on-call responders lookup.
    attributes: Any = None
    # Unique identifier of this on-call responders lookup.
    id: Any = None
    # Relationships for a schedule's on-call responders lookup, including the schedule and its responder groups.
    relationships: Any = None
    # Represents the resource type for a schedule's grouped on-call responders across the previous, current, and next positions.
    type: Any = None

@dataclasses.dataclass
class OnCallRespondersIncluded_Included_Attributes:
    position: Any = None

@dataclasses.dataclass
class OnCallRespondersIncluded_Included_Relationships:
    shifts: Any = None

@dataclasses.dataclass
class OnCallRespondersIncluded_Included:
    attributes: Any = None
    id: Any = None
    relationships: Any = None
    type: Any = None

@dataclasses.dataclass
class OnCallRespondersIncludedConfig:
    include: Any = None
    schedule_id: Any = None

@dataclasses.dataclass
class OnCallRespondersIncludedAttrs:
    # The main data object representing a schedule's on-call responders lookup, including relationships and metadata.
    data: Any = None
    include: Any = None
    # Related resources referenced in the responder groups' relationships, such as shifts, schedules, and users.
    included: Any = None
    schedule_id: Any = None

OnCallRespondersIncluded = ubx.DataSourceBinding(
    wire_type="datadog_schedule_on_call_responders_included",
    fields={
        "include": ubx.FieldSpec(wire_name="include"),
        "schedule_id": ubx.FieldSpec(wire_name="schedule_id"),
    },
)
