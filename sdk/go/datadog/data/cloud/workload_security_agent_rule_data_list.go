// Code generated by `ubx sdk gen` from datadog_cloud_workload_security_agent_rule_data_list. DO NOT EDIT.
package cloud

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type WorkloadSecurityAgentRuleDataList_Data_Attributes_Actions_Hash struct {
	Field any
}

type WorkloadSecurityAgentRuleDataList_Data_Attributes_Actions_Kill struct {
	Signal any
}

type WorkloadSecurityAgentRuleDataList_Data_Attributes_Actions_Metadata struct {
	ImageTag any
	Service any
	ShortImage any
}

type WorkloadSecurityAgentRuleDataList_Data_Attributes_Actions_Set struct {
	Append any
	DefaultValue any
	Expression any
	Field any
	Inherited any
	Name any
	Scope any
	Size any
	Ttl any
	Value any
}

type WorkloadSecurityAgentRuleDataList_Data_Attributes_Actions struct {
	Disabled any
	Filter any
	Hash any
	Kill any
	Metadata any
	Set any
}

type WorkloadSecurityAgentRuleDataList_Data_Attributes_Creator struct {
	Handle any
	Name any
}

type WorkloadSecurityAgentRuleDataList_Data_Attributes struct {
	Actions any
	AgentConstraint any
	AgentVersion any
	Blocking any
	Category any
	CreationAuthorUuId any
	CreationDate any
	Creator any
	DefaultRule any
	Description any
	Disabled any
	Enabled any
	Every any
	Expression any
	Filters any
	GroupId any
	Monitoring any
	Name any
	ProductTags any
	Silent any
	UpdateAuthorUuId any
	UpdateDate any
	UpdatedAt any
	Updater any
	Version any
}

type WorkloadSecurityAgentRuleDataList_Data struct {
	Attributes any
	Id any
	Type any
}

type WorkloadSecurityAgentRuleDataListConfig struct {
	PolicyId any
}

type WorkloadSecurityAgentRuleDataListAttrs struct {
	// A list of Agent rules objects
	Data any
	PolicyId any
}

var WorkloadSecurityAgentRuleDataList = ubx.DataSourceBinding{
	WireType: "datadog_cloud_workload_security_agent_rule_data_list",
	Fields: ubx.FieldMap{
		"PolicyId": ubx.FieldSpec{WireName: "policy_id"},
	},
}
