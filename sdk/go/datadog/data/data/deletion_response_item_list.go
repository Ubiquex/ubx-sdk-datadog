// Code generated by `ubx sdk gen` from datadog_data_deletion_response_item_list. DO NOT EDIT.
package data

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type DeletionResponseItemList_Data_Attributes struct {
	CreatedAt         any
	CreatedBy         any
	CustomerMessage   any
	DisplayedTotal    any
	ErrorCategory     any
	FromTime          any
	Indexes           any
	IsCreated         any
	OrgId             any
	Product           any
	Query             any
	StartingAt        any
	Status            any
	ToTime            any
	TotalUnrestricted any
	UpdatedAt         any
}

type DeletionResponseItemList_Data struct {
	Attributes any
	Id         any
	Type       any
}

type DeletionResponseItemList_Meta struct {
	// The total deletion requests created by product.
	CountProduct any
	// The total deletion requests created by status.
	CountStatus any
	// The next page when searching deletion requests created in the current organization.
	NextPage any
	// The product of the deletion request.
	Product any
	// The status of the executed request.
	RequestStatus any
}

type DeletionResponseItemListConfig struct {
	NextPage any
	PageSize any
	Product  any
	Query    any
	Status   any
}

type DeletionResponseItemListAttrs struct {
	// The list of data deletion requests that matches the query.
	Data any
	// The metadata of the data deletion response.
	Meta     any
	NextPage any
	PageSize any
	Product  any
	Query    any
	Status   any
}

var DeletionResponseItemList = ubx.DataSourceBinding{
	WireType: "datadog_data_deletion_response_item_list",
	Fields: ubx.FieldMap{
		"NextPage": ubx.FieldSpec{WireName: "next_page"},
		"PageSize": ubx.FieldSpec{WireName: "page_size"},
		"Product":  ubx.FieldSpec{WireName: "product"},
		"Query":    ubx.FieldSpec{WireName: "query"},
		"Status":   ubx.FieldSpec{WireName: "status"},
	},
}
