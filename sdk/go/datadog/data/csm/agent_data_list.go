// Code generated by `ubx sdk gen` from datadog_csm_agent_data_list. DO NOT EDIT.
package csm

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type AgentDataList_Data_Attributes struct {
	AgentVersion any
	AwsFargate any
	ClusterName any
	DatadogAgent any
	EcsFargateTaskArn any
	Envs any
	HostId any
	Hostname any
	InstallMethodInstallerVersion any
	InstallMethodTool any
	IsCsmVmContainersEnabled any
	IsCsmVmHostsEnabled any
	IsCspmEnabled any
	IsCwsEnabled any
	IsCwsRemoteConfigurationEnabled any
	IsRemoteConfigurationEnabled any
	Os any
}

type AgentDataList_Data struct {
	Attributes any
	Id any
	Type any
}

type AgentDataList_Meta struct {
	// The index of the current page in the paginated results.
	PageIndex any
	// The number of items per page in the paginated results.
	PageSize any
	// Total number of items that match the filter criteria.
	TotalFiltered any
}

type AgentDataListConfig struct {
	// The sort direction for results.
	OrderDirection any
	Page any
	Query any
	Size any
}

type AgentDataListAttrs struct {
	// A list of Agents.
	Data any
	// Metadata related to the paginated response.
	Meta any
	// The sort direction for results.
	OrderDirection any
	Page any
	Query any
	Size any
}

var AgentDataList = ubx.DataSourceBinding{
	WireType: "datadog_csm_agent_data_list",
	Fields: ubx.FieldMap{
		"OrderDirection": ubx.FieldSpec{WireName: "order_direction"},
		"Page": ubx.FieldSpec{WireName: "page"},
		"Query": ubx.FieldSpec{WireName: "query"},
		"Size": ubx.FieldSpec{WireName: "size"},
	},
}
