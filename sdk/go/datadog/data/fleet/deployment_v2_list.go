// Code generated by `ubx sdk gen` from datadog_fleet_deployment_v2_list. DO NOT EDIT.
package fleet

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type DeploymentV2List_Data_Attributes_ConfigOperations struct {
	FileOp   any
	FilePath any
	Patch    any
}

type DeploymentV2List_Data_Attributes struct {
	Author              any
	ConfigOperations    any
	DurationSeconds     any
	ErrorSummary        any
	EstimatedFinishedAt any
	FinishedAt          any
	IsScheduled         any
	Query               any
	ScheduleId          any
	StartedAt           any
	Status              any
	TargetVersions      any
	TotalHosts          any
	UpdateType          any
}

type DeploymentV2List_Data struct {
	Attributes any
	Id         any
	Type       any
}

type DeploymentV2List_Meta_Page struct {
	// Total number of deployments available across all pages.
	TotalCount any
	// Total number of deployments matching the current filter query.
	TotalFilteredCount any
}

type DeploymentV2List_Meta struct {
	// Pagination details for the v2 list of deployments.
	Page any
}

type DeploymentV2ListConfig struct {
	Ascending  any
	Filter     any
	PageNumber any
	PageSize   any
	Sort       any
}

type DeploymentV2ListAttrs struct {
	Ascending any
	// Array of deployments matching the query criteria.
	Data   any
	Filter any
	// Metadata for the v2 list of deployments, including pagination information.
	Meta       any
	PageNumber any
	PageSize   any
	Sort       any
}

var DeploymentV2List = ubx.DataSourceBinding{
	WireType: "datadog_fleet_deployment_v2_list",
	Fields: ubx.FieldMap{
		"Ascending":  ubx.FieldSpec{WireName: "ascending"},
		"Filter":     ubx.FieldSpec{WireName: "filter"},
		"PageNumber": ubx.FieldSpec{WireName: "page_number"},
		"PageSize":   ubx.FieldSpec{WireName: "page_size"},
		"Sort":       ubx.FieldSpec{WireName: "sort"},
	},
}
